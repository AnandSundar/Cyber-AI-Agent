"""
Cyber AI Agent - Main Module
This module orchestrates a cognitive threat hunt by analyzing logs from Azure Log Analytics
using OpenAI's GPT models to detect potential security threats.
"""
...
# Example: I'm worried that windows-target-1 might have been maliciously logged into
# in the last few days

# Return an object describing the user's request and where/how
...
print(
    f"{Fore.WHITE}Cognitive hunt complete. Took {elapsed:.2f} seconds "
    f"and found {Fore.LIGHTRED_EX}{findings_count} {Fore.WHITE}potential threat(s)!\n"
)
...
query_is_about_nsg = query_context["about_network_security_group"]
...
for threat in hunt_results["findings"]:
    ...
    if query_is_about_individual_host:
        ...
        # If machine is already isolated, don't isolate again
        # (wastes API calls)
        if threat_confidence_is_high and (not machine_is_isolated):
            ...
            confirm = (
                input(
                    f"{Fore.RED}{Style.BRIGHT}Would you like to isolate this VM? (yes/no): "
                    + Style.RESET_ALL
                )
                .strip()
                .lower()
            ...
            if confirm.startswith("y"):
                ...
                # )
                machine_is_isolated = True  # Placeholder until quarantine is enabled
                if machine_is_isolated:
                    print(
                        Fore.GREEN + "[+] VM successfully isolated." + Style.RESET_ALL
                    )
                    print(
                        Fore.CYAN
